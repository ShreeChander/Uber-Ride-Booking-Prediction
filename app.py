...
city = {
    "Chennai Lighthouse": "Chennai Lighthouse",
    "Express Avenue Mall": "Express Avenue Mall",
    "PVR Ampa SkyOne": "PVR Ampa SkyOne",
    "Marina Beach": "Marina Beach",
    "Semmozhi Poonga": "Semmozhi Poonga",
    "Sai Baba Temple Mylapore": "Sai Baba Temple Mylapore"
}
...
cheapest_ride_pipeline = joblib.load('cheapest_ride.pkl')
...
set_background_image("imagesplaces/sun.jpg") 

# Call the function with the path to your image


def get_top5_cheapest_ride(
    ride_max_persons,
    distance_meters,
    lat_route_from,
    lon_route_from,
    ...
    # Define ride types and hour range
    ride_types = ['Uber Go', 'Go Sedan']
    hours = range(7, 24)  # 7 AM to 11 PM
    ...
    for ride in ride_types:
        for hour in hours:
            combinations.append({
                ...
                'lat_route_from': lat_route_from,
                'lon_route_from': lon_route_from,
                'lat_route_to': lat_route_to,
                'lon_route_to': lon_route_to
            })

    # Convert to DataFrame and predict
    future_df = pd.DataFrame(combinations)
    future_df['predicted_price'] = cheapest_ride_pipeline.predict(future_df)

    # Return top 5 cheapest
    return future_df.sort_values(by='predicted_price').head(5)[
        ['ride_type', 'hour', 'day_of_week', 'predicted_price']
    ]
...
# set_background("imagesplaces/marina.jpg")
# Display the image
# st.image(rounded_image, use_container_width=True)
...
try:
    ...
    with col1:
        ...
        hour_options = [f"{i}:00" for i in range(7,24)]
    ...
    with col2:
        city_names = list(city.values())

        # Default selections
        default_route_from = "Chennai Lighthouse"
        default_route_to = "Express Avenue Mall"

        # Dropdown for "Route from"
        route_from = st.selectbox("Route from", city_names, index=city_names.index(default_route_from))

        # Filter out the selected "route_from" from the "route_to" options
        route_to_options = [name for name in city_names if name != route_from]

        # Set default index for route_to (safely)
        default_index_to = 0
        if default_route_to in route_to_options:
            default_index_to = route_to_options.index(default_route_to)

        # Dropdown for "Route to"
        route_to = st.selectbox("Route to", route_to_options, index=default_index_to)
        ...
        st.write(f"Distance (Kilometers): {distance / 1000:.2f} Km")
    ...
    if lat_from and lon_from and lat_to and lon_to:
        ...
        if is_start_within_radius and is_end_within_radius:
            if st.button("Predict"):
                ...
                cheapest = get_top5_cheapest_ride(ride_max_persons= ride_max_persons,
                                        distance_meters=distance,
                                        lat_route_from=lat_from,
                                        lon_route_from =lon_from,
                                        lat_route_to=lat_to,
                                        lon_route_to=lon_to,
                                        ride_waiting_time=waiting_current,
                                        ride_time_minutes =0,
                                        day_of_week=day_of_week)
                ...
                # Style section (only run once)
                # Add styling first (separate call)
                # Custom CSS styles
                with st.container():
                    st.markdown("### **Top 5 Cheapest Ride Predictions**")

                    # Create 1 column layout to hold the list of rides
                    ride_columns = st.columns(1)

                    # Iterate over the top 5 rides and display them in a nice structured way
                    for _, row in cheapest.iterrows():
                        with ride_columns[0]:
                            # Inline display using custom HTML
                            st.markdown(f"""
                                <div style="display: flex; justify-content: space-between; padding: 5px;">
                                    <span><strong>Ride Type:</strong> {row['ride_type']}</span>
                                    <span><strong>Hour:</strong> {row['hour']}:00</span>
                                    <span><strong>Predicted Price:</strong> Rs. {row['predicted_price']:.2f}</span>
                                </div>
                            """, unsafe_allow_html=True)
